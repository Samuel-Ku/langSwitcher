...
LangSwitcher also uses **Input Monitoring** to detect the double-Shift / double-Option hotkey and custom shortcuts while other apps are focused.

1. Go to **System Settings → Privacy & Security → Input Monitoring**
2. Make sure LangSwitcher is in the list and **toggled on**
3. Restart LangSwitcher after granting

Without it, hotkeys may not fire in other applications — the Permissions tab in Settings shows the live status of both permissions.

::callout{icon="i-lucide-info" color="blue"}
Text conversion itself works through Accessibility; Input Monitoring only affects hotkey detection. If conversion works from the menu bar but the hotkey does nothing, Input Monitoring is the likely culprit.
::
